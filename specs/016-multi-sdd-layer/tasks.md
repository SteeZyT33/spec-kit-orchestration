...
- [x] T034 PHASE-1.5-DEFERRED: Introduce `NormalizedReviewEvidence` and `NormalizedWorktreeLane` types in `src/speckit_orca/sdd_adapter.py` so adapters do not have to know `flow_state` internals. Today `NormalizedArtifacts.review_evidence` is typed `Any` and carries the `flow_state.ReviewEvidence` dataclass directly, and `worktree_lanes` carries `flow_state.WorktreeLane`. A second adapter (OpenSpec, Phase 2) would either duplicate those imports or reverse-engineer the shapes; both are wrong. Build the normalized types in the adapter module, map `SpecKitAdapter` onto them, and have `to_feature_evidence` translate back to the legacy `flow_state` types at the boundary. **Required before adding any Phase 2 adapter.** **Done on `016-phase-1.5-normalized-types`: `NormalizedReviewSpec`, `NormalizedReviewCode`, `NormalizedReviewPr`, `NormalizedReviewEvidence`, `NormalizedWorktreeLane` added; `NormalizedArtifacts.review_evidence` retyped; `SpecKitAdapter.load_feature` populates adapter-owned types; `to_feature_evidence` translates at the boundary; T016 golden parity gate still byte-exact; 482 tests pass (+11 new).**
