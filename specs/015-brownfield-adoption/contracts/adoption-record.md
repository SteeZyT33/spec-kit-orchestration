...
- `review_state` is hard-coded to `"not-applicable"` for the
  adoption kind. **This field is new to flow-state**; it does not
  exist in current `flow_state.py` runtime (see "Runtime status"
  at the top of this contract). 015 introduces it inline on the
  adoption view; 013's contract separately introduces it on the
  spec-lite view (`unreviewed | self-reviewed | cross-reviewed`).
  Whichever runtime ships first owns adding the field to the
  flow-state view dict; the second runtime extends the value set.
  015 does NOT modify 012's per-artifact Review Milestone fields
  (`review_spec_status`, `review_code_status`, `review_pr_status`,
  `overall_ready_for_merge`) — those are 012's aggregator contract,
  orthogonal to this view field.
...
parser (no YAML frontmatter), matching the serialization approach
used by evolve, brainstorm-memory, and spec-lite contracts.

**Important divergence from 013's parser**: 013's spec-lite parser
is **strict** on section shape (exact heading text for all 5 body
sections; no provision for unknown sections). 015's adoption parser
is **tolerant** — recognized sections require exact headings +
relative order; unknown sections land in an `extra` bucket without
raising; status / optional-section pairings are not enforced. This
difference is deliberate: ARs are operator-editable reference
documents over an indefinite lifespan, where strict parsing would
penalize hand edits. Implementers copying 013's parser shape as a
starting point MUST add the tolerant-parser affordances — they are
not free.
