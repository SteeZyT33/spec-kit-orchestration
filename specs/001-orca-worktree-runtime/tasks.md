...
- [ ] T001 Create `templates/worktree-registry.example.json` with the initial Orca registry schema
- [ ] T002 Update `templates/worktree-record.example.json` to match the runtime lane record schema exactly
- [ ] T003 [P] Review `docs/worktree-protocol.md` and `docs/delivery-protocol.md` against the runtime scope; note any mismatches to address during implementation
...
- [ ] T004 Create `scripts/bash/orca-worktree-lib.sh` with repo-root, default-branch, branch-name, and path-resolution helpers
- [ ] T005 Implement registry and lane-record read/write helpers in `scripts/bash/orca-worktree-lib.sh`
- [ ] T006 Implement validation helpers in `scripts/bash/orca-worktree-lib.sh` for required fields, duplicate lane IDs, duplicate branches, and invalid target paths
- [ ] T007 Implement current-lane and feature-lane discovery helpers in `scripts/bash/orca-worktree-lib.sh`
...
- [ ] T008 [US1] Create `scripts/bash/orca-worktree.sh` with argument parsing and subcommand routing for `create`, `list`, `status`, and `cleanup`
- [ ] T009 [US1] Implement nested-worktree detection and git repository safety checks in `scripts/bash/orca-worktree.sh`
- [ ] T010 [US1] Implement path computation and target-path validation for worktree creation in `scripts/bash/orca-worktree.sh`
- [ ] T011 [US1] Implement default-branch restoration logic before worktree creation in `scripts/bash/orca-worktree.sh`
- [ ] T012 [US1] Implement git worktree creation plus lane-record and registry updates in `scripts/bash/orca-worktree.sh`
- [ ] T013 [US1] Add clear create-flow output and next-step instructions in `scripts/bash/orca-worktree.sh`
- [ ] T014 [US1] Manually verify create flow in this repository and record any runtime adjustments needed
...
- [ ] T015 [US2] Implement metadata-first `list` output in `scripts/bash/orca-worktree.sh`
- [ ] T016 [US2] Implement `status` output in `scripts/bash/orca-worktree.sh` for current lane, sibling lanes, and registry drift warnings
- [ ] T017 [US2] Add drift detection between registry metadata and `git worktree list` in `scripts/bash/orca-worktree.sh`
- [ ] T018 [US2] Update `README.md` with a short worktree runtime section and example usage for `list` and `status`
- [ ] T019 [US2] Manually verify list/status behavior against valid metadata and intentionally drifted metadata
...
- [ ] T020 [US3] Implement merged/retired cleanup candidate detection in `scripts/bash/orca-worktree.sh`
- [ ] T021 [US3] Implement safe worktree removal and metadata update logic in `scripts/bash/orca-worktree.sh`
- [ ] T022 [US3] Implement warnings for ambiguous or active cleanup candidates in `scripts/bash/orca-worktree.sh`
- [ ] T023 [US3] Manually verify cleanup behavior with merged, retired, and ambiguous lane states
...
- [ ] T024 [P] Ensure `scripts/bash/orca-worktree-lib.sh` and `scripts/bash/orca-worktree.sh` are executable and documented consistently
- [ ] T025 Update `docs/worktree-protocol.md` and `docs/delivery-protocol.md` if runtime behavior changed the protocol details
...
- [ ] T027 Run the full manual verification flow one more time and summarize the evidence in the feature artifacts or commit notes
