# Plan: Orca YOLO Runtime — Single-Lane Execution Driver
...
(spec, run-stage-model, run-state, orchestration-policies, Lane
Agent Binding). `014-yolo-runtime` is the **implementation** — a
resumable single-lane driver built on those contracts. 009 stays
authoritative; 014 adds runtime code, tests, and the event log.
**Research inputs**:
...
- [`specs/012-review-model/plan.md`](../012-review-model/plan.md)
  (new review vocabulary 014 must consume)
...
This plan consumes **012's new review vocabulary** (`review-spec`,
`review-code`, `review-pr`) since 012 plan is now on main. 009's
run-stage-model references the old four-review names; that gets
collapsed as part of 012's runtime rewrite and 014 targets the new
vocabulary from day one.

## 2. Strawman answers (8 open questions)

Leans documented here for edit-don't-answer-cold review.

| # | Question | Strawman answer |
|---|---|---|
| 1 | New 014 spec vs extend 009 | **New 014 spec.** 009 stays contract-only, 014 owns runtime. Keeps history clean and lets 009 stay stable while runtime churns. |
| 2 | Event log layout | **Per-run directory**: `.specify/orca/yolo/runs/<run-id>/events.jsonl`. Matches spec-kitty's per-feature layout. Easier single-run inspection. |
| 3 | ULID library vs inline | **Inline.** ULID is ~40 lines of Python. Adding a dependency for 40 lines is wrong — same rule I'd apply to any other tiny utility. |
| 4 | Stale-run thresholds | **Configurable with defaults: 3 days `claimed` / 7 days `in-progress`.** Shorter than spec-kitty's 7/14 because Orca's single-operator usage is tighter. Configurable via runtime config file or CLI flag. |
| 5 | Review vocabulary alignment | **Align with 012.** 014 targets `review-spec`/`review-code`/`review-pr` from day one. 009's old four-review names are collapsed as part of 012's runtime rewrite. |
| 6 | Cross-mode auto-selection | **Yes, automatic with explicit override allowed.** Matches 012 plan's cross-pass agent routing policy: always different agent, prefer highest tier, downgrade on timeout, no same-agent fallback. 012's policy is the source of truth; 014 calls into it via matriarch. |
| 7 | Matriarch dual-write | **Yes.** yolo event log is the yolo-local source of truth; matriarch mailbox events are the supervision channel. Dual-write is cheap and avoids coupling the two storage backends. |
| 8 | Resume drift detection | **Warn on `head_commit_sha` mismatch.** When resume loads the event log, it compares the last-recorded `head_commit_sha` against the current HEAD. Mismatch triggers a warning and a prompt for operator confirmation; automatic reconciliation is explicitly rejected because drift is exactly the kind of thing Orca should never silently fix. |

**Question 4 and 6 are worth extra attention.** Edit 4 if you want
tighter or looser stale thresholds. Edit 6 if you want to weaken
the "always different agent" rule (but see 012 plan section 6 —
012 already locked this in, so 014 just honors it).
...
- Worktree creation at the `implement` stage, cleanup at terminal
...
- Cross-pass routing via 012's policy (calls matriarch for agent selection)
- `flow_state.py` extension to read yolo run state for features that
  have an active yolo run
- `matriarch.py` extension to consume yolo events in supervised mode
...
- `specs/009-orca-yolo/tasks.md` walk-through: check off the tasks
  014 runtime satisfies (T001 spec, T002 run-stage-model, T003
  run-state, etc.)

### Explicitly out of scope

- **Command prompt body for `commands/yolo.md`.** Stub file only.
  Full prompt rewritten in a separate follow-up task after the
  plan and contracts land. Same deferral rule as 012 and 013.
- Mission integration branch model — spec-kitty has one, Orca does
  not need it for single-lane
...
- Replacing flow-state as the "where is this feature now?"
  aggregator — yolo writes into its event log; flow-state continues
  to be the visible aggregator
- Supporting spec-lite as a yolo start artifact — explicitly
  excluded from v1 per 013 brainstorm
- Heartbeat-based liveness detection — relies on timestamp
  heuristic instead

## 4. The execution loop shape
...
- **Added `clarify`** between `specify` and `review-spec` — spec-
  kit's `/speckit.clarify` is mandatory per the clarify integration
  research doc
...
012's runtime rewrite will update 009's run-stage-model contract
to match. 014 targets the new vocabulary directly.
...
- `event_id` — ULID, 26 chars, lex-sortable, generated inline (no
  dependency)
- `run_id` — the yolo run this event belongs to
- `event_type` — enum (see below)
- `timestamp` — RFC3339 UTC, `Z` offset required per 010 event-envelope
- `lamport_clock` — integer, monotonic per writer
- `actor` — agent or human identifier (`claude`, `codex`, `user:<handle>`, etc.)
...
- `evidence` — optional list of artifact paths (test output, review
  artifact, etc.)
...
not happen. The dedup rule protects against double-commit bugs and
future multi-writer scenarios.

## 6. State reducer

`reduce(events: list[Event]) → RunState`

**Pure function.** No I/O, no side effects, no external state. Same
...
- Events are sorted by `(lamport_clock, timestamp, event_id)`
- Duplicate event_ids are dropped
- Each event updates exactly one field group, enforced by a match statement per event_type
- Invalid transitions (e.g., `terminal → stage_entered`) are silently ignored with a warning logged — the reducer never errors, it just rejects nonsense
...
**Pure function.** Computes what should happen next from the current
state. The caller (agent or human) then executes and reports back.
...
- `outcome == "paused"` → `paused` (caller decides to resume or cancel)
- Stage == `implement` and `review_code_status == "pending"` →
  `step` pointing at review-code
- Stage == `review-code` and cross pass missing → `step` pointing at
  cross-pass with agent selected via 012's policy (call into matriarch)
- Stage == `plan` and `review_spec_status != "complete"` →
  `decision_required` with prompt "review-spec must run before plan"
- Stage == `specify` and spec.md has no `## Clarifications` →
  `decision_required` with prompt pointing at `/speckit.clarify`
- Stage == `pr-create` and all reviews complete →
  `step` pointing at PR creation
- All other transitions follow the 009 + 012 run-stage-model

The rules are not exhaustive here; full coverage lives in the
contract file `contracts/decision-rules.md` (follow-up task).
...
- **At `implement` stage entry**: yolo calls into
  `scripts/bash/orca-worktree.sh` (or directly into
  `src/speckit_orca/spec_lite.py`'s future runtime — TBD) to create
  a worktree for the feature branch and writes its path into the
  run state. Implement-stage events record the worktree path.
...
### Stale-claim gotcha (from spec-kitty research)

The research doc flagged stale-claim false positives: a
legitimately-paused agent (waiting for human input) can get
flagged as stale. 014 mitigates by:
...
3. Compare against materialized `status.json` snapshot — if they
   disagree, event log wins and snapshot is regenerated with a warning
4. Validate worktree state (if `implement` stage active):
   - Branch matches expected
   - `head_commit_sha` matches last recorded
   - If mismatch → warn and require operator confirmation (per
     strawman answer 8)
...
- **No automatic code validation inside the worktree.** The worktree
  is trusted as-is; if the operator modified code outside a yolo
  event, that modification is preserved and carried into the next
  event.

## 10. Matriarch supervised mode
...
- Stage advancement emits a matriarch event-envelope entry in
  addition to the yolo event log (**dual write** per strawman
  answer 7), so matriarch's readiness aggregation stays current
...
`matriarch_sync_failed`. A future `yolo repair --run <id>` command
replays the missing matriarch writes; not in v1 scope.

### Standalone mode

Standalone mode skips everything in this section. No lane binding,
no mailbox, no dual writes, no lane registry consultation on
resume. `mode` is recorded in the first `run_started` event and is
...
- **`specs/014-yolo-runtime/contracts/event-envelope.md`** —
  event shape contract (already drafted in this plan's section 5,
  formalized in follow-up task)

- **`specs/014-yolo-runtime/contracts/run-state.md`** — RunState
  dataclass contract, reducer rules

- **`specs/014-yolo-runtime/contracts/decision-rules.md`** — full
  decision rule table mapping (state, context) → Decision

- **`specs/014-yolo-runtime/contracts/worktree-lifecycle.md`** —
  when worktrees are created, cleaned up, marked orphan

- **`specs/014-yolo-runtime/contracts/resume-protocol.md`** —
  the resume read path, drift detection, stale handling

- **`specs/014-yolo-runtime/contracts/supervised-mode.md`** —
  dual-write protocol, matriarch reconciliation rules, failure
  handling

- **`specs/014-yolo-runtime/data-model.md`** — RunState, Event,
  Decision entities with field-level documentation

- **`specs/014-yolo-runtime/quickstart.md`** — walked-through
  example of a full run from start through pr-create
...
- **`specs/009-orca-yolo/tasks.md`** — walk the 29 tasks and check
  off the ones 014 runtime satisfies. Already partially done in
  PR #23's tasks reconciliation pass, but PR #23 only covered 010.
  009's tasks need the same treatment once 014 ships.
...
- **`.gitignore`** — add `.specify/orca/yolo/runs/` if runs should
  not be committed (they probably should NOT be committed — they're
  machine-local ephemeral state; but the materialized `status.json`
  snapshots might be worth committing for resumability across
  collaborators. Open question.)
...
- `specs/009-orca-yolo/spec.md` — contract layer stays as-is
- `specs/009-orca-yolo/contracts/*` — 009's contracts unchanged;
  014 references them from its own contract files

## 12. Rollout sequence

014 is the biggest wave yet. The rollout is sequenced across
multiple PRs because no single PR should touch all of yolo.py plus
flow-state plus matriarch plus tests plus contracts plus commands.

**Suggested PR sequence:**

### PR A — Contracts and data-model

All files under `specs/014-yolo-runtime/contracts/`, plus
`data-model.md` and `quickstart.md`. No code changes. This is the
"lock the shape before writing the code" PR.
...
Wire yolo.py into `scripts/bash/orca-worktree.sh` (or whatever
becomes the canonical worktree creation path). Implement
...
Walk 009's 29 tasks and check off what 014 satisfies. Small PR,
doc-only.

**Six or seven PRs over a few sessions.** Each is reviewable
independently. The core runtime PR (B) is the biggest; the others
are narrower.
...
### Tests specifically NOT in scope for 014 v1
...
- **012 plan merged** ✅ (PR #29 on main) — provides the review
  vocabulary 014 consumes
- **docs/research/spec-kitty-execution-loop.md on main** ✅ (PR #28)
  — research grounding for harvest decisions

### Soft prerequisites (nice to have before PR B — core runtime)

- **012 contracts merged** — 014's review-code event integration
  is cleaner if the artifact shape is locked. Can proceed without
  by stubbing to the plan's section 6, but contracts make it
  tighter.
- **013 plan merged** — only matters for documentation. 013 doesn't
  gate 014.

### What 014 blocks

- **Nothing external.** 014 is the final runtime wave in the
  current upgrade program.

### Suggested sequencing across waves

1. 012 contracts (independent task, already queued)
2. 013 contracts (independent task, already queued)
3. **014 PR A** — contracts + data-model + quickstart (needs
   012 contracts ideally but not strictly)
4. **014 PR B** — core runtime
5. 012 runtime rewrite (flow-state + matriarch + 009 vocabulary)
6. **014 PR C** — flow-state integration (depends on 012 runtime)
7. **014 PR D** — matriarch integration (depends on 012 runtime)
8. **014 PR E** — worktree lifecycle
9. **014 PR F** — command prompt
10. **014 PR G** — 009 tasks reconciliation

Realistic timeline: 014 is probably 4-6 sessions of focused work
across PRs B through F. PR A and PR G are quick. The core runtime
(PR B) is the biggest individual chunk.
...
- Stage transitions match 009's run-stage-model (updated to 012's
  review vocabulary)
...
- Resume reproduces the correct current state from event log
  ...
  or missing
- Stale detection fires at configurable thresholds without heartbeats
...
- All 45+ existing tests still pass after the runtime lands
...
## 17. Open questions for the contract-writing task
...
1. **`.specify/orca/yolo/runs/` gitignore**: machine-local or
   ...
   `status.json` snapshots so collaborators can resume each other's
   runs if needed. Or just gitignore everything and accept runs
   are local-only.
2. **Event envelope fields list**: the plan's section 5 is the
...
5. **Resume across branch checkouts**: if the operator changes
   ...
   detection (strawman answer 8). If the SHA doesn't match, the
   resume warns and requires confirmation.

## 18. Suggested next steps

1. You react to the eight strawman answers in section 2 and the
   five contract-writing open questions in section 17
2. Merge this plan PR (after review)
3. Start **PR A** — contracts + data-model + quickstart under
   `specs/014-yolo-runtime/`
4. Start **PR B** — core runtime in `src/speckit_orca/yolo.py`
   plus `tests/test_yolo.py`
5. Continue through PRs C-G per the rollout sequence above
6. After PR G merges, 009 tasks.md should be mostly checked off
   and 014 runtime is live
